...
if(isset($_POST['confirm'])){
    $transf=$_POST['transf'];
    echo $transf;
    header("Location: transferfrom.php?tf=$transf");
...
<body>
    ...
    <div class="container">
        <form action="transferfrom.php" method="post">
            <div class="form-group"><br><br><br>
                <label for="name">Transfer From:</label>
                <?php
                echo '<select class="form-control" name="transf">';
                $sql="SELECT * FROM `userdata` WHERE balance>0";
            ...
            <center> <button type="submit" name="confirm" class="btn btn-success">Confirm</button></center>
        </form> <br><br><br><br>
         <?php
         if(isset($_GET['tf'])){
             $from = $_GET['tf'];
             $sql="SELECT * FROM `userdata` WHERE sno=$from";
             $result=mysqli_query($conn, $sql);
             while($row=mysqli_fetch_assoc($result)){
                // echo $row['name'].'user is selected.';
               ...
               echo '<a class="btn btn-success" href="transfer.php?tf='.$row['sno'].'" role="button">Proceed</a>'; 
            }
